...
public class Noun {
    ...
    private final int declension; // 0 will be indeterminable.
    private int typeOfDeclension = 0; // 0 will be indeterminable in a
    // declension.
    private final String[] aCases; // form strings the cases.
        ...
        if (aDictionaryForm.length >= 2) {
            ...
            switch (aDictionaryForm[1]) {
                ...
                case "ii":
                    ...
                    break;
            } // end switch on genitive termination.

        } // end if there are at least two indexes in the array of dictionary form after splitting by ", -".
    ...
    private String decline1() {
        ...
        for (int i = 0; i < aCases.length; i++) {
            singular.append(aCases[i]).append(" &lt;i>").append(theme).append(aTerminationsSingular[i]).append("&lt;/i>&lt;br>");
        } // end for singular.
...
// Now we format the string including the singular and the plural if needed:
    ...
    public void makeParadigm() {
        // A kind of switch on declension:
        // First declension:
        if (declension == 1) {
